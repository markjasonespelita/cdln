...
@section('content')
    ...
    {{-- <div class="container-fluid bg-primary mb-5">
        <div class="d-flex flex-column align-items-center justify-content-center" style="min-height: 400px">
            <h3 class="display-3 font-weight-bold text-white">Announcement Detail</h3>
        </div>
    </div> --}}
    <!-- Header End -->
            ...
            <div class="col-lg-8">
                <div class="d-flex flex-column text-left mb-3">
                    ...
                    <div class="d-flex">
                        ...
                        <p class="mr-3"><i class="fa fa-comments text-primary"></i> {{ App\Models\Announcementcomments::where('announcements_id', $item->id)->count() }}</p>
                    </div>
                </div>
                <div class="mb-5">
                    @php
                    ...
                    @if ($file)
                        @if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'webp']))
                            {{-- 🖼️ Image --}}
                            <img class="img-fluid rounded w-100 mb-4 border shadow-sm" src="{{ url('storage/announcements/'.$file) }}" alt="Image">

                        @elseif (in_array($extension, ['mp4', 'webm', 'ogg']))
                            {{-- 🎥 Video --}}
                            <div class="ratio ratio-16x9 mb-4">
                                <video controls class="rounded shadow-sm w-100">
                                    <source src="{{ url('storage/announcements/'.$file) }}" type="video/{{ $extension }}">
                        ...
                        @elseif ($extension === 'pdf')
                            ...
                            <div class="border rounded shadow-sm mb-4" style="height: 600px;">
                                <iframe src="{{ url('storage/announcements/'.$file) }}" class="w-100 h-100 rounded" style="border: none;"></iframe>
                            </div>
                        ...
                        @elseif (in_array($extension, ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx']))
                            ...
                            <div class="text-center p-4 bg-light rounded border shadow-sm mb-4">
                                ...
                                <a href="{{ url('storage/announcements/'.$file) }}" target="_blank" class="btn btn-outline-primary btn-sm">Download File</a>
                            </div>
                        ...
                        @else
                            ...
                            <div class="text-center p-4 bg-light rounded border shadow-sm mb-4">
                                ...
                                <a href="{{ url('storage/announcements/'.$file) }}" target="_blank" class="btn btn-outline-secondary btn-sm">Download File</a>
                            </div>
                        @endif
                    @endif
                ...
                <div class="mb-5 mx-n3">
                    <h2 class="mb-4 ml-3">Other Announcements</h2>
                    <div class="owl-carousel post-carousel position-relative">
                        @forelse (App\Models\Announcements::where('status', 'published')->get() as $announcement)
                            <div class="d-flex align-items-center bg-light shadow-sm rounded overflow-hidden mx-3">
                                <div class="pl-3">
                                    <h5 class=""><a href="{{ url('announcement-details/'.$announcement->id) }}">{{ $announcement->title }}</a></h5>
                                    <div class="d-flex">
                                        {{-- <small class="mr-3"><i class="fa fa-user text-primary"></i> Admin</small>
                                        <small class="mr-3"><i class="fa fa-folder text-primary"></i> Web Design</small> --}}
                                        <small class="mr-3"><i class="fa fa-comments text-primary"></i> {{ App\Models\Announcementcomments::where('announcements_id', $announcement->id)->count() }}</small>
                                    </div>
                                </div>
                            </div>
                        @empty
                ...
                <!-- Comment List -->
                <div class="mb-5">
                    <h2 class="mb-4">{{ App\Models\Announcementcomments::where('announcements_id', $item->id)->count() }} Comments</h2>

                    @forelse (App\Models\Announcementcomments::where('announcements_id', $item->id)->get() as $comment)
                                        ...
                                        <div class="modal fade" id="editModal{{ $comment->id }}" tabindex="-1" aria-labelledby="editModal{{ $comment->id }}Label" aria-hidden="true">
                                            <div class="modal-dialog">
                                                ...
                                                <div class="modal-body">
                                                    <form action='{{ route('announcementcomments.update', $comment->id) }}' method='POST'>
                                                        @csrf
                                        ...
                                        <div class="modal fade" id="deleteModal{{ $comment->id }}" tabindex="-1" aria-labelledby="deleteModal{{ $comment->id }}Label" aria-hidden="true">
                                            <div class="modal-dialog">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <a href="{{ route('announcementcomments.destroy', $comment->id) }}"><button type="button" class="btn btn-primary">Yes, Proceed</button></a>
                                                </div>
                ...
                <div class="bg-light p-5">
                    ...
                    <form action="{{ route('announcementcomments.store', $item->id) }}" method="POST">
                        @csrf
                        <div class="form-group">
