...
@section('content')
    <div class='row'>
        ...
        <div class='col-lg-6 col-md-6 col-sm-12' style='text-align: right;'>
            <a href='{{ url('trash-departments') }}'><button class='btn btn-danger'><i class='fas fa-trash'></i> Trash <span class='text-warning'>{{ App\Models\Departments::where('isTrash', '1')->count() }}</span></button></a>
            <a href='{{ route('departments.create') }}'><button class='btn btn-success'><i class='fas fa-plus'></i> Add Departments</button></a>
        </div>
    </div>
            ...
            <div class='row'>
                <div class='mt-2 col-lg-4 col-md-4 col-sm-12'>
                    <div class='row'>
                        ...
                        <div class='col-8'>
                            <form action='{{ url('/departments-paginate') }}' method='get'>
                ...
                <div class='mt-2 col-lg-4 col-md-4 col-sm-12'>
                    <form action='{{ url('/departments-filter') }}' method='get'>
                        <div class='input-group'>
                    ...
                    <tbody>
                        @forelse($departments as $item)
                            <tr>
                                ...
                                <td>{{ $item->name }}</td>
                                <td>
                                    <a href='{{ route('departments.show', $item->id) }}'><i class='fas fa-eye text-success'></i></a>
                                    <a href='{{ route('departments.edit', $item->id) }}'><i class='fas fa-edit text-info'></i></a>
                                    <a href='{{ route('departments.delete', $item->id) }}'><i class='fas fa-trash text-danger'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-delete').click(function () {
                ...
                $.post('/departments-delete-all-bulk-data', {
                    ids: array,
            ...
            $('.bulk-move-to-trash').click(function () {
                ...
                $.post('/departments-move-to-trash-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    window.location.reload();
