                ...
                <table class='table table-striped'>
                    <thead>
                        <tr>
                            ...
                            <th>#</th>
                            <th>Title</th><th>Description</th><th>Start_date</th><th>End_date</th><th>All_day</th><th>Location</th><th>Category</th><th>Users_id</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        @forelse($events as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->title }}</td><td>{{ $item->description }}</td><td>{{ $item->start_date }}</td><td>{{ $item->end_date }}</td><td>{{ $item->all_day }}</td><td>{{ $item->location }}</td><td>{{ $item->category }}</td><td>{{ $item->users_id }}</td>
                                <td>
                                    <a href='{{ route('events.restore', $item->id) }}'><i class='fas fa-recycle text-info'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-restore').click(function () {
                ...
                $.post('/events-restore-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    console.log(res)
