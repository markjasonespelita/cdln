...
@section('content')
    <div class='row'>
        ...
        <div class='col-lg-6 col-md-6 col-sm-12' style='text-align: right;'>
            <a href='{{ url('trash-events') }}'><button class='btn btn-danger'><i class='fas fa-trash'></i> Trash <span class='text-warning'>{{ App\Models\Events::where('isTrash', '1')->count() }}</span></button></a>
            <a href='{{ route('events.create') }}'><button class='btn btn-success'><i class='fas fa-plus'></i> Add Events</button></a>
        </div>
    </div>
            ...
            <div class='row'>
                <div class='mt-2 col-lg-4 col-md-4 col-sm-12'>
                    <div class='row'>
                        ...
                        <div class='col-8'>
                            <form action='{{ url('/events-paginate') }}' method='get'>
                    ...
                    <tbody>
                        @forelse($events as $item)
                            <tr>
                                ...
                                <td>{{ Smark\Smark\Dater::humanReadableDateWithDayAndTime($item->start_date) }}</td>
                                <td>{{ Smark\Smark\Dater::humanReadableDateWithDayAndTime($item->end_date) }}</td>
                                <td>
                                    @if($item->all_day == 1)
                                ...
                                <td>
                                    <a href='{{ route('events.show', $item->id) }}'><i class='fas fa-eye text-success'></i></a>
                                    <a href='{{ route('events.edit', $item->id) }}'><i class='fas fa-edit text-info'></i></a>
                                    <a href='{{ route('events.delete', $item->id) }}'><i class='fas fa-trash text-danger'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-delete').click(function () {
                ...
                $.post('/events-delete-all-bulk-data', {
                    ids: array,
            ...
            $('.bulk-move-to-trash').click(function () {
                ...
                $.post('/events-move-to-trash-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    window.location.reload();
