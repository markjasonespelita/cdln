    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form action="{{ route('events.update', $item->id) }}" method="POST">
                ...
                {{-- Start Date & Time --}}
                @php
                    $startParts = explode(' ', $item->start_date);
                ...
                {{-- End Date & Time --}}
                @php
                    $endParts = explode(' ', $item->end_date);
                    $endDate = $endParts[0] ?? '';
                    $endTime = $endParts[1] ?? '';
                @endphp
                <div class="row mb-3">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="checkbox" id="all_day" name="all_day" value="1" {{ $item->all_day == 1 ? 'checked' : '' }}>
                ...
                <a href="{{ route('events.index') }}" class="btn btn-outline-secondary mt-3 ms-2">Cancel</a>
            </form>
        </div>
    </div>
