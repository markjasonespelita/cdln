                ...
                <table class='table table-striped'>
                    <thead>
                        <tr>
                            ...
                            <th>#</th>
                            <th>Title</th><th>Slug</th><th>Content</th><th>Category</th><th>Attachment_path</th><th>Published_at</th><th>Expires_at</th><th>Status</th><th>Users_id</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        @forelse($announcements as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->title }}</td><td>{{ $item->slug }}</td><td>{{ $item->content }}</td><td>{{ $item->category }}</td><td>{{ $item->attachment_path }}</td><td>{{ $item->published_at }}</td><td>{{ $item->expires_at }}</td><td>{{ $item->status }}</td><td>{{ $item->users_id }}</td>
                                <td>
                                    <a href='{{ route('announcements.restore', $item->id) }}'><i class='fas fa-recycle text-info'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-restore').click(function () {
                ...
                $.post('/announcements-restore-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    console.log(res)
